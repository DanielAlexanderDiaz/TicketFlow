...
@router.get("/listar", response_model=list[InformacionTicket])
def listar_tickets_usuario(db: DBSession, usuario: UsuarioActual):
    return TicketService(db).ticket_by_usuario(usuario.id)

@router.get("/listar_todos", response_model=list[InformacionTicket], dependencies=[Depends(requiere_admin)])
...
@router.post("/", response_model=InformacionTicket, status_code=status.HTTP_201_CREATED, dependencies=[Depends(puede_gestionar_ticket)])
...
@router.get("/", response_model=InformacionTicket)
...
@router.patch("/{id_ticket}", response_model=InformacionTicket, dependencies=[Depends(puede_gestionar_ticket)])
...
@router.patch("/{id_ticket}/vigencia", response_model=InformacionTicket, dependencies=[Depends(puede_gestionar_ticket)])
def actualizar_ticket_activo(id_ticket: int, payload: TicketActivo, db: DBSession, usuario: UsuarioActual):
    return TicketService(db).actualizar_ticket_activo(id_ticket, usuario.id, payload)

@router.patch("/{id_ticket}/imagen", response_model=InformacionTicket, dependencies=[Depends(puede_gestionar_ticket)])
...
@router.patch("/{id_ticket}/estado", response_model=InformacionTicket, dependencies=[Depends(puede_gestionar_ticket)])
def actualizar_cambiar_estado(db: DBSession, id_ticket: int, new_estado: CambioEstadoTicket, usuario: UsuarioActual):
    return TicketService(db).cambio_estado_ticket(id_ticket, new_estado)
