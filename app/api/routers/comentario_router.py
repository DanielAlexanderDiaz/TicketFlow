...
@router.get("/", response_model=InformacionComentario)
...
@router.get("/{id_ticket}", response_model=list[InformacionComentario])
...
@router.get("/usuario/{id_usuario}", response_model=list[InformacionComentario])
def info_comentarios(db: DBSession, usuario: UsuarioActual):
    return ComentarioService(db).comentario_by_usuario(usuario.id)

@router.post("/{id_ticket}", response_model=InformacionComentario, status_code=status.HTTP_201_CREATED)
...
@router.patch("/{id_comentario}", response_model=InformacionComentario)
